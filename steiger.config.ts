...
export default defineConfig([
  ...
  {
    // `excessive-slicing` для features отключаем: карта переноса экранов
    // (FRONTEND.md §15, §16) — это ~20 узких «одно действие = один слайс» фич
    // (create-*/edit-*/manage-*), и добавление CRUD уроков (TЗ §15 п.9) выводит
    // счётчик за порог 20. Группировать их в под-папки (features/lesson/*) —
    // отдельный рефактор с массовой правкой импортов в widgets; узкие
    // слайсы-действия здесь сознательная форма, а не долг.
    files: ["./src/features/**"],
    rules: {
